describe 'using landlord within an engine' do
  ...
  context 'when Landlord.db_migrate_tenants is false' do
    it 'should not enhance tasks' do
      Landlord.db_migrate_tenants = false
      expect(Landlord::RakeTaskEnhancer).to_not receive(:enhance_task).with('db:migrate')
