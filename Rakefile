...
namespace :db do
  ...
  task :copy_credentials do
    require 'fileutils'
    landlord_db_file = 'spec/config/database.yml'
    rails_db_file = 'spec/dummy/config/database.yml'

    FileUtils.copy(landlord_db_file + '.sample', landlord_db_file, :verbose => true) unless File.exists?(landlord_db_file)
...
# TODO clean this up
def config
  Landlord::Test.config['connections']
end
